...
export class RegisterComponent implements OnInit {
  ...
  emailCheckUnique1() {
    this.analystService
      .checkUniqueEmail(this.userModel1.userEmail)
      .subscribe((res) => {
  ...
  emailCheckUnique2() {
    this.analystService
      .checkUniqueEmail(this.userModel2.userEmail)
      .subscribe((res) => {
        this.UserEmailCheck = res;
        if (this.UserEmailCheck != null) {
          this.emailAlreadyExist = "Email Already Exist";
  ...
  sendVerificationEmail(email) {
    window.alert(
      "We are sending an OTP to your Email, to verify this really is you!"
    );
    this.emailid = email;
    this._registerService.sendotp(email).subscribe(
      (data) => {
        this.otpno = data;
        if (this.otpno != null) {
      ...
      (error) => {
        this.error = error;
        window.alert(
          "We are facing some issues in sending OTP, please make sure you have entered correct email id!"
        );
      }
    );
  }
  ...
  onSubmit1() {
    ...
    this._registerService.registerPerson(this.personModel).subscribe(
      (data) => {
